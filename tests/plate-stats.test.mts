...
test('stores public aggregate stats by plate without storing visitor buckets', async (context) => {
  ...
  await Promise.all(
    Array.from({ length: 3 }, (_, index) =>
      store.record('abc 123', `2026-08-02T12:00:0${index}.000Z`),
    ),
  );
  ...
  assert.deepEqual(await store.get('ABC 123'), {
    ...
    previousCheckedAt: '2026-08-02T12:00:01.000Z',
  });
  ...
  assert.equal('status' in persisted['ABC 123'], false);
  assert.equal('message' in persisted['ABC 123'], false);
});
